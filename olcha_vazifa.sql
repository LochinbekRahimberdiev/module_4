...
(
    ...
    PRIMARY KEY (about_pr_id, about_key_id, about_value_id)
);
...
INSERT INTO olcha.category (title, image)
...
INSERT INTO olcha."group" (title, image, group_id)
VALUES ('Smartphones', 'smartphones.jpg', 1),
       ('Laptops', 'laptops.jpg', 1),
       ('Fiction', 'fiction.jpg', 2),
       ('Non-Fiction', 'non_fiction.jpg', 2),
       ('Male Clothing', 'mens_clothing.jpg', 3);

INSERT INTO olcha.product (name, price, discount, group_id)
VALUES ('iPhone 15 Pro Max', 1099.99, 50.00, 1),
...
INSERT INTO olcha.value_pr (text)
VALUES ('Apple'),
       ('Samsung'),
       ('Black'),
       ('White'),
...
$$
    ...
    BEGIN
        ...
        FOR prod IN SELECT id FROM olcha.product
            LOOP
                ...
                VALUES (prod.id, brand_id, (SELECT id
                                            FROM olcha.value_pr
                                            WHERE text = (CASE WHEN random() < 0.5 THEN 'Apple' ELSE 'Samsung' END)));

                -- Model (randomly assign a model name based on a simple heuristic)
                model_id := (SELECT id FROM olcha.key_pr WHERE text = 'Model');
                INSERT INTO olcha.about_pr (about_pr_id, about_key_id, about_value_id)
                VALUES (prod.id, model_id, (SELECT id
                                            FROM olcha.value_pr
                                            WHERE text = (CASE WHEN random() < 0.5 THEN 'iPhone 14' ELSE 'Galaxy S21' END)));

                -- Color (randomly assign between Black and White)
                color_id := (SELECT id FROM olcha.key_pr WHERE text = 'Color');
                INSERT INTO olcha.about_pr (about_pr_id, about_key_id, about_value_id)
                VALUES (prod.id, color_id, (SELECT id
                                            FROM olcha.value_pr
                                            WHERE text = (CASE WHEN random() < 0.5 THEN 'Black' ELSE 'White' END)));

                -- Storage (randomly assign between 128GB and 256GB)
                storage_id := (SELECT id FROM olcha.key_pr WHERE text = 'Storage');
                INSERT INTO olcha.about_pr (about_pr_id, about_key_id, about_value_id)
                VALUES (prod.id, storage_id, (SELECT id
                                              FROM olcha.value_pr
                                              WHERE text = (CASE WHEN random() < 0.5 THEN '128GB' ELSE '256GB' END)));

                -- Warranty (randomly assign between 1 Year and 2 Years)
                warranty_id := (SELECT id FROM olcha.key_pr WHERE text = 'Warranty');
                INSERT INTO olcha.about_pr (about_pr_id, about_key_id, about_value_id)
                VALUES (prod.id, warranty_id, (SELECT id
                                               FROM olcha.value_pr
                                               WHERE text = (CASE WHEN random() < 0.5 THEN '1 Year' ELSE '2 Years' END)));
            END LOOP;
    END
$$;
...
VALUES ('Jahon', 'jony@gmail.com', 'Chotki narsa ekan. Oka, dastavka bormi?', 4),
       ...
       ('Gulsanam', 'guly@gmail.com', 'Vay, vashshe zor narsa ekanu.', 3),
       ('Fozil', 'foziljon@gmail.com', 'Juda foydali buyum ekan 😊', 5),
       ('Rozigul', 'rozaxon@gmail.com', 'Puliga arziydimi bu?', 1),
       ('Niyat', 'niyatboykemilbayev@gmail.com', 'Bizdin qazagistangayam keledimi?', 2),
       ('Hoshimboy', 'qalpoqcha@gmail.com', 'O’zbekcha yozsa bo’lmiydimi?', 6);
